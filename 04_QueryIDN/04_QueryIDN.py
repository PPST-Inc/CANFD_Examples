...
QUERY_IDN_DBC_PATH = os.path.join(SCRIPT_DIR,'04_QueryIDN.dbc')
...
class QueryIDN():
    ...
    def __init__(self):
        ...
        try:
            self.database = cantools.db.load_file(QUERY_IDN_DBC_PATH)
        except:
        ...
        writeResult = self.WriteSCPICommand("*IDN?")
        ...
        print("")
        reqResult = self.RequestFirmwareVersion()
        if reqResult != PCAN_ERROR_OK:
            print("Error requesting Firmware Version message.")
    ...
    def RequestFirmwareVersion(self):
        """Request the message 'Firmware_Version_Message' and read the response

        To request a message we send an empty CAN frame with the with the desired ID
        and wait for the response.
        The signal 'Firmware_Version' is an array of ASCII chars. The response value
        is decode as a String.

        Returns:
          A TPCANStatus error code
        """

        message = self.database.get_message_by_name('Firmware_Version_Message')
        signal = message.get_signal_by_name('Firmware_Version')
        ...
        for i in range(message.length):
            msgCanMessage.DATA[i] = 0 # dummy data

        # Send a message to request the Firmware Version
        stsResult = self.m_objPCANBasic.Write(self.PcanHandle, msgCanMessage)
        ## Checks if the message was sent
        if (stsResult != PCAN_ERROR_OK):
            return stsResult
        ...
        while tries > 0:
            ...
            if stsResult == PCAN_ERROR_OK:
                msgRead = result[1]
                # If the frame ID read is not the requested
                # we read again
        ...
        # Decode the byte array of the CAN frame to string
        firmware_version = str(msgRead.DATA, encoding='utf-8')
        # Strip the tailing null values
        firmware_version = firmware_version.rstrip('\x00')

        print("Response message:")
        print(f"  {message.name}")
        print(f"  signal: {signal.name}: {firmware_version}")
